...
private:
    ...
    void add_proc_symbol(ProcImpl* p)
    {
        ...
        for(iter = p->m_decl_list -> begin(); iter != p->m_decl_list -> end(); ++iter){
            //for(auto iter2 = ((*iter)->m_symname_list) -> begin(); iter2 != ((*iter)->m_symname_list) -> end(); ++iter2)
            DeclImpl* q = dynamic_cast<DeclImpl*>(*iter);
           // cout << "### our len is "<<q->m_symname_list->size();
            for(int h=0;h<q->m_symname_list->size();h++){
    ...
    void check_call(Call *p)
    {
        ...
        if(p->m_expr_list->size() != s->m_arg_type.size()){
            cout << "error mismatch sizes " << p->m_expr_list->size() << " " << s->m_arg_type.size()<<endl;
...
public:
    ...
    void visitTString(TString* p)
    {
        ...
        cout << "###len is " << p->m_primitive->m_data<<endl;
